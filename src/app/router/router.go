...
func Init(init *config.Injector) *gin.Engine {
	...
	/** These API requests are forwarded to another service */
	anotherAPI := router.Group("/api/v1/another")
	{
		targetStr := os.Getenv("ANOTHER_BACKEND_TARGET")
		url, _ := url.Parse(targetStr)
		proxy := httputil.NewSingleHostReverseProxy(url)
		anotherAPI.Any("/*any", func(c *gin.Context) {
